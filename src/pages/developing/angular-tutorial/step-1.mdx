...
To create a new Angular project with Angular CLI, just run in the parent directory of `carbon-tutorial-angular`:

```bash
ng new carbon-tutorial-angular
```
...
Within the project folder `carbon-tutorial-angular`, the `src` directory should
...
**Note:** You will get compilation errors because the `@carbon/icons` module doesn't have typescript typings.
I created a file `index.d.ts` with a line `declare module '@carbon/icons/*';` and added `"index.d.ts"` as an
include in the file `tsconfig.json`
...
**Note** the above in not correct.  `src/app/app.component.spec.ts`, and `src/app/header/header.component.spec.ts` 
do not have `AppModule` and have not imported the `IconService`.
...
<ibm-header name="Carbon Tutorial Angular">
  ...
  <ibm-header-global>
    <ibm-header-action title="action">
      <svg ibmIcon="notification" size="16"></svg>
    </ibm-header-action>
    <ibm-header-action title="action">
      <svg ibmIcon="user--avatar" size="16"></svg>
    </ibm-header-action>
    <ibm-header-action title="action">
      <svg ibmIcon="app-switcher" size="16"></svg>
    </ibm-header-action>
  </ibm-header-global>
</ibm-header>
```
